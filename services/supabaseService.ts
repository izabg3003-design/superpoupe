...
const safeGetEnv = (key: string): string | undefined => {
  ...
  // Tenta várias fontes comuns de injeção de variáveis de ambiente no browser
  const sources = [
  ...
  for (const source of sources) {
    ...
    for (const k of keysToTry) {
      if (typeof source[k] === 'string' && source[k].length > 0) {
        return source[k];
      }
    }
  }

  // Fallbacks diretos para as chaves fornecidas pelo utilizador para garantir funcionamento imediato
...
export async function fetchProductsFromCloud(searchTerm?: string, category?: string, store?: StoreId) {
  ...
  try {
    ...
    console.error("Erro fetch cloud:", e);
    return [];
  }
}
